...
```
XGBoost gives an error rate of 10.8%. This is better than a single decision tree, but does not beat Random Forest or SVM. The threshold analysis graph above shows that the best misclassification rate for XGBoost is actually at the threshold = .4. 
...
lines(x = 1:10 / 10, y = NNErrors)
```
We get abetter misclassification rate from the PCA version of NN using 13 variables of `r round(PCA_13_v_NeuralNetMisClassRate*100,2)`%, which is `r round(100*(NeuralNetMisClassRate - PCA_13_v_NeuralNetMisClassRate),2)`% less than our last Neural Network, and an optimal cutoff threshold of .5. 
...
lines(x = 1:10 / 10, y = NNErrors)
```
As expected, this version has the lowest misclassification rate of `r round(PCA_20_v_NeuralNetMisClassRate*100,2)`%, which is `r round((PCA_13_v_NeuralNetMisClassRate-PCA_20_v_NeuralNetMisClassRate)*100,2)`% lower than the 13 variable PCA NN, and `r round((NeuralNetMisClassRate-PCA_20_v_NeuralNetMisClassRate)*100,2)`% lower than the original neural network. As you can see, however, we are reaching a point of diminishing returns; adding ~15% more variation only decreased the error rate by 
`r round((PCA_13_v_NeuralNetMisClassRate-PCA_20_v_NeuralNetMisClassRate)*100,2)`%
...
```
KNN produces an error of `r round(AvgError_best_knn*100,2)`%, with an optimal k-value of 4
...
```
Unsurprisingly, clustering has the largest error, of `r round(AvgErrorClustering*100,2)`%; given this is an unsupervised method and our data has labels. 
...
print(error_output)
```
Random Forest has the lowest misclassification rate. We will remake this model using the full dataset.


#5 Make predictions

Remake the random forest model using the full dataset:
```{r}
