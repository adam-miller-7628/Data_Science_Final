...
sizeTestingSeet = dim(testingSet)[1]
propC = sum(trainingSet[, 1] == "CONFIRMED") / sizeTrainingSet #the proportion of the training set that is confirmed
print(propC)
sample(c("CONFIRMED","FALSE POSITIVE"), sizeTestingSet,replace = TRUE, prob = c(propC,1-propC))
```
...
```
Unsurprisingly, clustering has the largest error, of `r round(AvgErrorClustering*100,2)`%; given this is an unsupervised method and our data has labels. Additionally, Clustering is especially terrible at predicting "FALSE POSITIVEs," with a false negative rate of `r round(Clustering_FN_Rate*100,2)`%
