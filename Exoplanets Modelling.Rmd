...
for (fold in 1:5) {
  ...
  BoostError = sum(round(BoostPredictions) != testingSet$koi_disposition)
  misclassificationRateBoost = error / sizeTestSet
...
PoV <- res.pca.exoplanets$sdev^2/sum(res.pca.exoplanets$sdev^2) #get proportions of variance
numPcas = 13
...
#fviz_pca_biplot(res.pca.exoplanets, col.ind = "cos2", repel = TRUE)
...
results = data.frame(actual = testingSet.norm.PCA$koi_disposition, prediction = predictedLabels$net.result)
error = sum(results$actual != round(results$prediction))
PCA_13_v_NeuralNetMisClassRate = error/sizeTestSet
```
...
results = data.frame(actual = testingSet.norm.PCA$koi_disposition, prediction = predictedLabels$net.result)
error = sum(results$actual != round(results$prediction))
PCA_20_v_NeuralNetMisClassRate = error/sizeTestSet
```



```{r}
error_output = data.frame("Model"= c("Decision Tree","GLM","Random Forest","SVM","Neural Net","KNN","Clustering", "PCA 13 Var NN", "PCA 20 Var NN"), "Misclassification Rate" = c(AvgErrorDT,AvgErrorGLM,AvgErrorRF,AvgErrorSVM,NeuralNetMisClassRate,AvgError_best_knn,AvgErrorClustering, PCA_13_v_NeuralNetMisClassRate,PCA_20_v_NeuralNetMisClassRate))
print(error_output)
```
